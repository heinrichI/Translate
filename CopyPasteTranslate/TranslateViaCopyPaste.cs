...
namespace CopyPasteTranslate
{
    ...
    {
        private readonly LiteralCollection _subtitle;
        private readonly LiteralCollection _subtitleOriginal;
        private bool _abort;
        public LiteralCollection TranslatedSubtitle { get; private set; }

        public TranslateViaCopyPaste(LiteralCollection literalCollection)
        {
            ...
            _subtitle = new LiteralCollection(literalCollection);
            _subtitleOriginal = new LiteralCollection(literalCollection);
        ...
        private void Translate(string source, string target, CopyPasteTranslator translator, int maxTextSize)
        {
            ...
            _abort = false;
            progressBarTranslate.Maximum = _subtitleOriginal.Literals.Count;
            ...
            try
            {
                ...
                {
                    var block = blocks[i];
                    using (var form = new TranslateBlock(block, string.Format("Translate block {0} of {1}", i + 1, blocks.Count), checkBoxAutoCopyToClipboard.Checked))
                    {
